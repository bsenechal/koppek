...
var path = require('path'),
  ...
  s3Credentials = require(path.resolve('s3Credentials.json')), 
  ...
  _ = require('lodash'),
  UserFunction = require(path.resolve('./modules/users/server/controllers/users/users.role.server.controller')),
  DealModification = mongoose.model('DealModification');

var snowball_stemmer = require(path.resolve('./node_modules/snowball-stemmer.jsx/dest/french-stemmer.common.js'));
var keyword_extractor = require('keyword-extractor');


// TODO : remettre le fichier de config
//var dealByPage = parseInt(config.dealByPage);
//var submitBeforeModif = parseInt(config.submitBeforeModif);

var dealByPage = parseInt(5);
var submitBeforeModif = parseInt(5);
...
function tagsManager(deal, action, next){
...
exports.getVisited  = function getVisited(idDeal, callback) {
...
function dealAlertManager (idDeal){
  Deal.getVisited(idDeal, function(visited){
