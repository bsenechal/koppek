...
angular.module('deals').controller('DealsController', ['$scope','$rootScope','$q', '$stateParams', '$resource', '$location', 'Deals', 'uuid4', 'Authentication', '$mdToast',
  function($scope,$rootScope, $q, $stateParams,$resource, $location, Deals, uuid4, Authentication, $mdToast) {
    var okscroll = 1;
    var limitDelta = 10;
    $scope.deals = null;
    $scope.limitStart = 0;
  ...
  $scope.editDeal = function() {
    ...
    console.log(dealModification);
        ...
        $resource('/api/addModification').save(dealModification, function(response) {
    ...
    if(image) {
         $resource('/api/deals/getS3Credentials').get(function(credential) {
...
console.log("là !");
    ...
    function updateGrade(deal,action) {
      ...
      $resource('/api/users/votes').save({_id: deal._id});
      ...
      $resource('/api/updateGrade').save({_id: deal._id, idUser:userId , action: action},function(dealResult) {
    ...
    */
    ...
    $scope.markersByRadius = function(){
      ...
      var markersByRadius = $resource(
          '/api/MarkersByRadius/:srchLng/:srchLat/:srchRadius'
        );
