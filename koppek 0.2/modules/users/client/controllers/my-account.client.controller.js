...
angular.module('users').controller('MyAccountController', ['$scope', 'Authentication', '$resource', '$location', '$mdToast', '$http', 'uuid4',
	function($scope, Authentication, $resource, $location, $mdToast, $http, uuid4) {
        ...
        $http.get("/api/getNumberOfComment").success(function(response) { $scope.nbOfComment = response; });
        
        $http.get("/api/getNumberOfDeal").success(function(response) { $scope.nbOfDeal = response; });
        ...
        $scope.editAvatar = function() {
			...
			$resource('/api/users/getS3Credentials').get(function(credential) {
